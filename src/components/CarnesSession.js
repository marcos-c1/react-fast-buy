...
const CarnesSession = ({ onAdd }) => {
    ...
    useEffect(() => {
        ...
        const getCarnes = async () => {
            const CarnesFromServer = await fetchCarnes()
            setCarnes(CarnesFromServer)
        } 
    ...
    const fetchCarnes = async () => {
        const res = await fetch('http://localhost:5000/carnes')
    ...
    return (
        <nav>
            <div className="datascroller-demo">
                <div className="card">
                    <DataScroller ref={ds} value={carnes} itemTemplate={itemTemplate} rows={5}
